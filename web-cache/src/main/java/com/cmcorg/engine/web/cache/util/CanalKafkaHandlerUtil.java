...
public class CanalKafkaHandlerUtil {
    ...
    @Nullable
    public ICanalKafkaHandlerKey getByKey(String key) {
        if (StrUtil.isBlank(key)) {
            return null;
        }
        for (ICanalKafkaHandlerKey item : I_CANAL_KAFKA_HANDLER_KEY_LIST) {
            if (key.equals(item.getKey(cacheProperties))) {
                return item;
            }
        }
        return null;
    ...
    public void putCanalKafkaHandlerMap(ICanalKafkaHandlerKey[] iCanalKafkaHandlerKeyArr) {
