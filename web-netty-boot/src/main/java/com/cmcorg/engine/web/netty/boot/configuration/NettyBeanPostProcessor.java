...
public class NettyBeanPostProcessor implements BeanPostProcessor {
    ...
    public Object postProcessAfterInitialization(@NotNull Object bean, String beanName) throws BeansException {
        ...
        RequestMapping controllerRequestMappingAnnotation = beanClass.getAnnotation(RequestMapping.class);

        StrBuilder strBuilder = StrBuilder.create();

        String controllerMappingName = "";
        if (controllerRequestMappingAnnotation != null) {
            controllerMappingName = controllerRequestMappingAnnotation.value()[0]; // 只取第一个路径
        }
        ...
        for (Method item : declaredMethodArr) {
            RequestMapping methodNettyMappingAnnotation = item.getAnnotation(RequestMapping.class);
            if (methodNettyMappingAnnotation == null) {
                continue;
            }
            String methodMappingName = methodNettyMappingAnnotation.value()[0]; // 只取第一个路径

            // 组装并处理路径
            List<String> splitTrimList = StrUtil
                .splitTrim(strBuilder.append(controllerMappingName).append(methodMappingName).toStringAndReset(), "/");
